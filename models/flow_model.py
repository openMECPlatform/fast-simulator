...
# rdgraph = gp.read_gpickle('new_sample_graph.gpickle')
...
numFlowList = [100, 200, 300, 400, 500]
# numFlowList = [100]
...
for numFlow in numFlowList:
    opt_total_cost_dict[numFlow] = list()
    opt_total_comm_cost_dict[numFlow] = list()
    opt_total_buff_cost_dict[numFlow] = list()
    opt_total_exec_time_dict[numFlow] = list()
    opt_migration_time_dict[numFlow] = list()
    opt_number_of_fami[numFlow] = 0
    opt_total_comp_dict[numFlow] = list()

    base_total_cost_dict[numFlow] = list()
    base_total_comm_cost_dict[numFlow] = list()
    base_total_buff_cost_dict[numFlow] = list()
    base_total_exec_time_dict[numFlow] = list()
    base_number_of_fami[numFlow] = 0

    bast_total_cost_dict[numFlow] = list()
    bast_total_comm_cost_dict[numFlow] = list()
    bast_total_buff_cost_dict[numFlow] = list()
    bast_total_exec_time_dict[numFlow] = list()
    bast_migration_time_dict[numFlow] = list()
    bast_number_of_fami[numFlow] = 0
    bast_total_comp_dict[numFlow] = list()

    last_total_cost_dict[numFlow] = list()
    last_total_comm_cost_dict[numFlow] = list()
    last_total_buff_cost_dict[numFlow] = list()
    last_total_exec_time_dict[numFlow] = list()
    last_migration_time_dict[numFlow] = list()
    last_number_of_fami[numFlow] = 0
    last_total_comp_dict[numFlow] = list()

    rast_total_cost_dict[numFlow] = list()
    rast_total_comm_cost_dict[numFlow] = list()
    rast_total_buff_cost_dict[numFlow] = list()
    rast_total_exec_time_dict[numFlow] = list()
    rast_migration_time_dict[numFlow] = list()
    rast_number_of_fami[numFlow] = 0
    rast_total_comp_dict[numFlow] = list()

    past_total_cost_dict[numFlow] = list()
    past_total_comm_cost_dict[numFlow] = list()
    past_total_buff_cost_dict[numFlow] = list()
    past_total_exec_time_dict[numFlow] = list()
    past_migration_time_dict[numFlow] = list()
    past_number_of_fami[numFlow] = 0
    past_total_comp_dict[numFlow] = list()

    retry = 0
    while retry < (maxRetry-1):
        print 'retry:', retry
        intial_graph = gp.read_gpickle('sample_graph.gpickle')
        # if retry == 4:
        ...
        while True:
            ...
            if count >= numFlow:            # Migration condition
                ...
                if opt_total_cost:
                    opt_total_cost_dict[numFlow].append(opt_total_cost)
                    opt_total_comm_cost_dict[numFlow].append(opt_comm_cost)
                    opt_total_buff_cost_dict[numFlow].append(opt_buff_cost)
                    opt_total_exec_time_dict[numFlow].append(opt_exec_time)
                    opt_migration_time_dict[numFlow].append(opt_total_mig_time)
                    opt_total_comp_dict[numFlow].append(opt_total_comp_cost)
                else:
                    opt_number_of_fami[numFlow] += 1

                if base_total_cost:
                    base_total_cost_dict[numFlow].append(base_total_cost)
                    base_total_comm_cost_dict[numFlow].append(base_comm_cost)
                    base_total_buff_cost_dict[numFlow].append(base_buff_cost)
                    base_total_exec_time_dict[numFlow].append(base_exec_time)
                else:
                    base_number_of_fami[numFlow] += 1

                if bast_total_cost:
                    bast_total_cost_dict[numFlow].append(bast_total_cost)
                    bast_total_comm_cost_dict[numFlow].append(bast_comm_cost)
                    bast_total_buff_cost_dict[numFlow].append(bast_buff_cost)
                    bast_total_exec_time_dict[numFlow].append(bast_exec_time)
                    bast_migration_time_dict[numFlow].append(bast_total_mig_time)
                    bast_total_comp_dict[numFlow].append(bast_total_comp_cost)
                else:
                    bast_number_of_fami[numFlow] += 1

                if last_total_cost:
                    last_total_cost_dict[numFlow].append(last_total_cost)
                    last_total_comm_cost_dict[numFlow].append(last_comm_cost)
                    last_total_buff_cost_dict[numFlow].append(last_buff_cost)
                    last_total_exec_time_dict[numFlow].append(last_exec_time)
                    last_migration_time_dict[numFlow].append(last_total_mig_time)
                    last_total_comp_dict[numFlow].append(last_total_comp_cost)
                else:
                    last_number_of_fami[numFlow] += 1

                if rast_total_cost:
                    rast_total_cost_dict[numFlow].append(rast_total_cost)
                    rast_total_comm_cost_dict[numFlow].append(rast_comm_cost)
                    rast_total_buff_cost_dict[numFlow].append(rast_buff_cost)
                    rast_total_exec_time_dict[numFlow].append(rast_exec_time)
                    rast_migration_time_dict[numFlow].append(rast_total_mig_time)
                    rast_total_comp_dict[numFlow].append(rast_total_comp_cost)
                else:
                    rast_number_of_fami[numFlow] += 1

                if past_total_cost:
                    past_total_cost_dict[numFlow].append(past_total_cost)
                    past_total_comm_cost_dict[numFlow].append(past_comm_cost)
                    past_total_buff_cost_dict[numFlow].append(past_buff_cost)
                    past_total_exec_time_dict[numFlow].append(past_exec_time)
                    past_migration_time_dict[numFlow].append(past_total_mig_time)
                    past_total_comp_dict[numFlow].append(past_total_comp_cost)
                else:
